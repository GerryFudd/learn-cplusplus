...
TEST(as_string)
{
  ...
  test::assert_equal<string>(test_int.as_decimal_string(), "4789");
}
...
{
  BigInt test_int;
  test::assert_equal<string>(test_int.as_decimal_string(), "0");
}
...
TEST(to_string_test_large)
{
  ...
  test::assert_equal<string>(result, "4294967296");;
}
...
TEST(to_string_test_large_complex)
{
  ...
  string result = test_int.as_decimal_string();
  test::assert_equal<string>(result, "-18446744073709551615");;
}
...
TEST(to_string_test_large_with_intermediate_zeros)
{
  ...
  string result = test_int.as_decimal_string();
  test::assert_equal<string>(result, "10000000001");;
}

TEST(to_hex_string_test)
{
  BigInt test_int((unsigned int []) { 0x4789 }, 1, false);
  test::assert_equal<string>(test_int.as_hex_string(), "0x4789");
}
...
{
  BigInt test_int;
  test::assert_equal<string>(test_int.as_hex_string(), "0x0");
}
...
TEST(to_hex_string_test_large)
{
  ...
  test::assert_equal<string>(result, "0x100000000");;
}
...
TEST(to_hex_string_test_large_complex)
{
  BigInt test_int((unsigned int []){4294967295, 4294967295}, 2, true);
  string result = test_int.as_hex_string();
  test::assert_equal<string>(result, "-0xffffffffffffffff");;
}
...
TEST(to_hex_string_test_large_with_intermediate_zeros)
{
  ...
  string result = test_int.as_hex_string();
  test::assert_equal<string>(result, "0x200000001");;
}
